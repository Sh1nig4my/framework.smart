...
import {
  DuplicateCaseJsonError,
  insertCaseJsonRecord,
  type CaseJsonRecord,
} from "@/app/server/repositories/case-json.repository";
import { getAllowedOptions, step1Schema } from "@/shared/lib/step1-json";
...
export type CaseJsonSubmissionResult = {
  requestId: string;
  caseFileName: string;
  caseJson: CaseJson;
};

export class InvalidCasePayloadError extends Error {
  constructor(message: string) {
    super(message);
  }
}

export class DuplicateCasePayloadError extends Error {
  constructor() {
    super("Esiste gia una richiesta identica con la stessa email e lo stesso case JSON.");
  }
}

export class DatabaseConfigurationError extends Error {
  constructor() {
    super("Configurazione database mancante. Imposta MONGODB_URI e riprova.");
  }
}
...
export function isHoneypotTriggered(payload: FormPayload) {
  return Boolean(normalizeString(payload[HONEYPOT_FIELD_NAME]));
}

export async function submitCaseJson(payload: FormPayload): Promise<CaseJsonSubmissionResult> {
  const caseJson = buildCaseJson(payload);
  const validationError = getValidationMessage(payload, caseJson);

  if (validationError) {
    throw new InvalidCasePayloadError(validationError);
  }
  ...
  try {
    const requestId = await insertCaseJsonRecord(record);

    return {
      requestId,
      caseFileName: buildCaseFileName(email),
      caseJson,
    };
  } catch (error) {
    if (error instanceof DuplicateCaseJsonError) {
      throw new DuplicateCasePayloadError();
    }
