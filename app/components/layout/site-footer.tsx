...
export function SiteFooter() {
  return (
    <footer className="mt-14 border-t border-[var(--line)] bg-white/85">
      <div className="mx-auto grid w-full max-w-6xl gap-8 px-6 py-10 md:grid-cols-[1.4fr_1fr] md:px-10">
        <div className="space-y-3">
          ...
          <p className="max-w-2xl text-sm leading-relaxed text-[var(--ink-700)]">
            Framework agentico no-code per progettazione, diffusione e gestione di iniziative formative con standard
            metodologici, sicurezza operativa e collaborazione uomo-AI.
          </p>
          <a
        ...
        <div className="space-y-3">
          ...
          <ul className="space-y-2 text-sm text-[var(--ink-700)]">
            {legalLinks.map((link) => (
              <li key={link.href}>
                <a href={link.href} className="transition hover:text-[var(--ink-900)]">
                  {link.label}
                </a>
              </li>
            ))}
