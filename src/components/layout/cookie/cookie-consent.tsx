  ...
  return (
    <div className="fixed right-4 bottom-4 z-50 w-full max-w-md rounded-2xl border border-[var(--line)] bg-white p-5 shadow-[0_18px_45px_-20px_rgba(7,21,35,0.65)]">
      ...
      <p className="mt-2 text-sm leading-relaxed text-[var(--ink-700)]">
        Questa app usa solo cookie tecnici necessari al funzionamento. Non utilizza cookie di profilazione o
        analytics in questo step.
      </p>
      <p className="mt-2 text-xs leading-relaxed text-[var(--ink-600)]">
        In caso di evoluzione futura della raccolta dati, policy e meccanismi di consenso verranno aggiornati prima
        dell&apos;attivazione.
      </p>
      <div className="mt-4 flex flex-wrap items-center gap-3">
        <button type="button" onClick={handleAccept} className="sf-button-primary">
          Ho capito
        </button>
        <a href="/cookie-policy" className="text-xs font-medium text-[var(--brand-700)] transition hover:text-[var(--ink-900)]">
          Leggi cookie policy
        </a>
      </div>
    </div>
